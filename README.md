...
https://youtu.be/KxQ4wF3SLFs?si=Z7xgRf3Sv3hVZyWP
...
## 📝 소개
[배포판의 사용 설명서](https://melissa-industry.gitbook.io/projectlucia_client/)
...
본 클라이언트의 원활한 구동을 위한 권장 및 최소 사양입니다.

| 항목 | 최소 사양 (Minimum) <br> `*음성 인식 미사용 기준` | 권장 사양 (Recommended) <br> `*모든 AI 기능 활성화 기준` |
| :--- | :--- | :--- |
| **운영체제 (OS)** | Windows 10 / 11 (64-bit 전용) | Windows 10 / 11 (64-bit 전용) |
...
| **저장 장치 (Storage)**| 1 GB 이상의 여유 공간 | 10 GB 이상의 여유 공간 (SSD 강력 권장) |
| **오디오 입력 (Audio)** | 없음 (텍스트 전용 대화 가능) | 깨끗한 수음이 가능한 마이크 필수 <br> (화자 인식 및 실시간 STT용) |
| **네트워크 (Network)** | 서버 및 SQL 연결을 위한 인터넷 환경 | 상시 안정적인 초고속 인터넷 환경 |

> ⚠️ **주의사항**
> * 본 프로그램은 **Windows OS 기반**으로 동작합니다. Linux, iOS, Android 환경에서는 동작을 보장하지 않습니다.
> * 클라이언트 실행 시 **기본 RAM 소모량은 약 200MB** 수준이나, 원활한 OS 멀티태스킹과 Live2D 렌더링 처리를 위해 최소 4GB 이상의 메모리가 확보되어야 합니다.
> * **내장 그래픽 환경:** STT를 미사용하거나 `Base Q5` 모델 적용 및 그래픽 설정을 `Low/VeryLow`로 설정 권장.
> * **외장 그래픽 환경:** 강력 추천 STT 모델인 `Large-v3-Turbo Q8` 모델 사용 시 약 2.0GB VRAM이 요구되므로, `High` 옵션 구동을 위해 4GB 이상의 VRAM이 필수입니다.
...
- **Live2d Prefab**: 캐릭터 교체 작업을 할 때 live2d 캐릭터의 Prefab 에 등록된 인스펙터를 반드시 확인하여 주시길 바랍니다.
   - Transform
   - Cubism Model(스크립트)
