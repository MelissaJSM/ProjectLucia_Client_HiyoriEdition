    ...
    public class WhisperWrapper
    {
        ...
        ~WhisperWrapper()
        {
            if (_whisperCtx == IntPtr.Zero)
                return;
            // Finalizer에서는 네이티브 리소스 해제만 시도
            // 하지만 Unity 에디터 종료 시점 등에서는 위험할 수 있으므로 주의
            // 여기서는 명시적 Dispose를 권장하며, Finalizer는 최후의 수단
            // WhisperNative.whisper_free(_whisperCtx); // <-- 잠재적 위험: 메인 스레드가 아닐 수 있음
        }
        ...
        public WhisperResult GetText(float[] samples, int frequency, int channels, WhisperParams param)
        {
            lock (_lock)
            {
                if (_disposed)
                {
                    LogUtils.Warning("WhisperWrapper is disposed. Skipping inference.");
                    return null;
                }
        ...
        {
            if (_shouldCancel || _disposed) return false;
            IsRunning = true;
            try
            {
            ...
            {
                LogUtils.Error($"InferenceWhisper error: {e.Message}");
                return false;
            }
            finally
            {
        ...
        private static void NewSegmentCallbackStatic(IntPtr ctx, IntPtr state, int nNew, IntPtr userDataPtr)
        {
            ...
            // 이미 Dispose 됐거나 Cancel 요청이 있으면 무시
            if (userData.Wrapper._disposed || userData.Wrapper._shouldCancel) 
                return;
        ...
        public void Dispose()
        {
            lock (_lock)
            {
                ...
                if (_whisperCtx != IntPtr.Zero)
                {
                    LogUtils.Log("Disposing Whisper context...");
                    WhisperNative.whisper_free(_whisperCtx);
                }

                _disposed = true;
                GC.SuppressFinalize(this); // Finalizer 생략
            }
        }
