...
public class TestReporter : MonoBehaviour
{
	...
	void Start()
	{
		...
		if(SettingData.IsDebug) Debug.Log("test long text sdf asdfg asdfg sdfgsdfg sdfg sfg" +
		...
		for (int i = 0; i < 10; i++) {
			if(SettingData.IsDebug) Debug.Log("Test Collapsed log");
			if(SettingData.IsDebug) Debug.LogWarning("Test Collapsed Warning");
			if(SettingData.IsDebug) Debug.LogError("Test Collapsed Error");
		}
	...
	void threadLogTest()
	{
		for (int i = 0; i < threadLogTestCount; i++) {
			if(SettingData.IsDebug) Debug.Log("Test Log from Thread");
			if(SettingData.IsDebug) Debug.LogWarning("Test Warning from Thread");
			if(SettingData.IsDebug) Debug.LogError("Test Error from Thread");
		}
	}
	...
	void Update()
	{
		...
		while (currentLogTestCount < logTestCount && drawn < 10) {
			if(SettingData.IsDebug) Debug.Log("Test Log " + currentLogTestCount);
			if(SettingData.IsDebug) Debug.LogError("Test LogError " + currentLogTestCount);
			if(SettingData.IsDebug) Debug.LogWarning("Test LogWarning " + currentLogTestCount);
			drawn++;
			currentLogTestCount++;
		}
		...
		if (elapsed >= 1) {
			elapsed = 0;
			if(SettingData.IsDebug) Debug.Log("One Second Passed");
		}
	}
