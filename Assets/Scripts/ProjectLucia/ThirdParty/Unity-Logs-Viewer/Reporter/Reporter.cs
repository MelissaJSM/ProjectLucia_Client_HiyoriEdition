...
public class Reporter : MonoBehaviour
{
	...
	public void Initialize()
	{
		if (!created) {
			...
			if(SettingData.IsDebug) Debug.LogWarning("tow manager is exists delete the second");
	...
	void Update()
	{
		...
		calculateStartIndex();
		if (!show && SettingData.IsDebug) {
			doShow();
		}
        else if (show && !SettingData.IsDebug) {
            doClose();
        }
	...
	void AddLog(string condition, string stacktrace, LogType type)
	{
		...
		if (TotalMemUsage > maxSize) {
			clear();
			if(SettingData.IsDebug) Debug.Log("Memory Usage Reach" + maxSize + " mb So It is Cleared");
			return;
		}
...
#if UNITY_CHANGE3
		currentScene = SceneManager.GetActiveScene().name ;
		if(SettingData.IsDebug) Debug.Log( "Scene " + SceneManager.GetActiveScene().name + " is loaded");
#else
		currentScene = Application.loadedLevelName;
		if(SettingData.IsDebug) Debug.Log("Scene " + Application.loadedLevelName + " is loaded");
#endif
	}
	...
	IEnumerator readInfo()
	{
		...
		if (!string.IsNullOrEmpty(www.error)) {
			if(SettingData.IsDebug) Debug.LogError(www.error);
		}
		else {
#if UNITY_CHANGE4
    ...
    private void SaveLogsToDevice()
    {
        ...
        if(SettingData.IsDebug) Debug.Log("Saving logs to " + filePath);
